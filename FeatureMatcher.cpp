...
void FeatureMatcher::setConfidenceLevel(double conf) {
    confidence= conf;
}

//Set MinDistanceToEpipolar
void FeatureMatcher::setMinDistanceToEpipolar(double dist) {
    distance= dist;
}

//Set ratio
void FeatureMatcher::setRatio(float rat) {
    ratio= rat;
}

// draw epipolar lines
void FeatureMatcher::drawEpipolarLines(cv::Mat& image_out, // output image
...
void FeatureMatcher::drawMatches(cv::Mat& image, // output image
...
void FeatureMatcher::drawPerspective(cv::Mat& image, // output image
                                            cv::Mat& image1, // image 1
                                            std::vector<cv::Point2f>& points1, // keypoints 1
                                            std::vector<cv::Point2f>& points2) // keypoints 2
{
...
// i.e. size will be 0)
int FeatureMatcher::ratioTest(std::vector<std::vector<cv::DMatch> >
                                     &matches) {
    int removed=0;
    // for all matches
...
void FeatureMatcher::symmetryTest(const std::vector<std::vector<cv::DMatch> >& matches1,
...
cv::Mat FeatureMatcher::ransacTest(const std::vector<cv::DMatch>& matches,
...
cv::Mat FeatureMatcher::match(cv::Mat& image2, // input scene image
...
cv::Mat FeatureMatcher::match(std::vector<cv::KeyPoint>& keypoints1, // input computed object keypoints
                              cv::Mat& descriptors1, // input computed object descriptors
...
void FeatureMatcher::setDetecter(cv::Ptr<cv::Feature2D> dt) {
    detector = dt;
}
